...
import { listSchemas, getSchema, getTemplates, GenerationTemplate, SchemaListItem } from "@/lib/api";
...
export function GenerateSection() {
  ...
  const [encryptionConfigs] = useState<EncryptionConfig[]>([{ id: "1", tableName: "", attribute: "", algorithm: "" }]);
  const [builderTables] = useState<SchemaBuilderTable[]>([{ id: "t1", name: "users", columns: [{ id: "c1", name: "id", type: "uuid", nullable: false, isPrimary: true, isUnique: true }] }]);
  ...
  const handleSchemaParsed = (data: unknown) => {
    const raw = data as Record<string, unknown>;
    const parsed = raw.databases ? (raw as unknown as ApiResponse) : JSON.parse(raw.data as string) as ApiResponse;
    setDatabaseStructure(parsed);
    const counts: Record<string, number> = {};
    parsed.databases.forEach((db) => db.tables.forEach((t) => counts[t.name] = 10));
  ...
  const loadExistingSchema = async (sid: string) => {
    ...
    } catch { /* error handled higher up or ignored */ }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[{ id: "upload", title: "Schema", icon: FileText }, { id: "configure", title: "Configure", icon: Zap }, { id: "generate", title: "Generate", icon: BarChart3 }].map((s) => (
        ...
        <div>
          {currentStep === "upload" && (
            schemaSource === "builder" ? (
              ...
              <SchemaStage onSchemaParsed={handleSchemaParsed} isUploading={isUploading} setIsUploading={setIsUploading} setUploadError={() => {}} savedSchemas={savedSchemas} loadExistingSchema={loadExistingSchema} schemaSource={schemaSource} setSchemaSource={setSchemaSource} />
            )
          )}
